...
function verify(p, s, h) {
  return crypto.timingSafeEqual(
    Buffer.from(hp(p, s).hash, "hex"),
    Buffer.from(h, "hex")
  );
}
...
async function schema() {
  if (!process.env.DATABASE_URL) {
    throw Error("DATABASE_URL não configurada");
  }
  ...
  try {
    ...
    if (
      ...
      if (!name || !email || pw.length < 6) {
        return out(res, 400, {
          error:
            "Nome, email e senha (mín. 6) são obrigatórios."
        });
      }
